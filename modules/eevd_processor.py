...
def process_eevd(input_path: str, output_dir: str, error_dir: str = "erro"):
    """
    Processa arquivo EEVD (Vendas Débito) — Splitter v4.2
    Suporta:
      • 01  RV Originais
      • 011 Cancelamentos
      • 05  CV detalhados
    ...
    stats_pv = defaultdict(lambda: {"qtd_rv":0,"qtd_cv":0,"bruto":0,"desconto":0,"liquido":0,
    ...
    for raw in detalhes:
        ...
        # -------------------------------------------------------------
        # Registros com impacto financeiro / PV
        # -------------------------------------------------------------
        if t == "01":  # Resumo de vendas
            pv = parts[1]
            rv_to_pv[parts[4]] = pv if len(parts)>4 else pv
        ...
        if t == "011":  # Cancelamento
            pv = parts[1] if len(parts) > 1 else None
            valor_cancel = to_centavos(parts[4]) if len(parts) > 4 else 0
            if pv:
                registros_por_pv[pv].append(parts)
                s = stats_pv[pv]
                s["qtd_cv"] += 1
                s["bruto"] -= valor_cancel
                s["liquido"] -= valor_cancel
            continue
    ...
    if not tem_movimento:
        ...
        with open(out_path,"w",encoding="utf-8") as f:
            ...
            trailer_sem = ["04", header_parts[1] if len(header_parts)>1 else "0",
                           _pad(0,6),_pad(0,6),
                           zeros15,zeros15,zeros15,zeros15,zeros15,zeros15,
                           _pad(2,6)]
    ...
    for pv, regs in registros_por_pv.items():
        ...
        reg02 = ["02", matriz_ou_grupo, _pad(s["qtd_rv"],3), _pad(s["qtd_cv"],6),
                 _val(s["bruto"]), _val(s["desconto"]), _val(s["liquido"]),
                 _val(s["bruto_pred"]), _val(s["desc_pred"]), _val(s["liq_pred"])]
        reg03 = reg02.copy(); reg03[0]="03"
        total_registros = len(linhas_out) + 3  # header + detalhes + 02/03/04
        reg04 = ["04", matriz_ou_grupo, _pad(s["qtd_rv"],6), _pad(s["qtd_cv"],6),
                 _val(s["bruto"]), _val(s["desconto"]), _val(s["liquido"]),
                 _val(s["bruto_pred"]), _val(s["desc_pred"]), _val(s["liq_pred"]),
